...
from beancount_tx_cleanup.cleaner import CLEANUP, TAG_DESTINATION, E, Extractors

AIB_EXTRACTORS: Extractors = {
    # transaction timestamp
    'time': (E(r'(?i)(time)? *(\d\d:\d\d)$', value=r'\g<2>'),),
    # amount of foreign currency
    'foreign-amount': (E(r' ([\d.]+) [A-Z]{3}@ [\d.]+', replacement=r'\g<0>'),),
    ...
    'payment-processor': (
        E(r'(?i)^(paypal|sumup|sq|sp|zettle)[ _]', transformer=lambda m: m.lower(),
          translation={
            'sq': 'square',
            'sp': 'stripe',
          }),
        E(r'(?i)^google +\b(?!google|cloud|commerce|domain|ireland|music|payment|play|servic|svcs|store|youtub|voic)(.+)',
          replacement=r'\g<1>', value='google'),
        E(r'(?i)^google +\b(payment|play|servic)', replacement=r'\g<0>', value='google'),
    ),
    CLEANUP: (
        # remove the date strings, unless it's describing a quarterly fee transaction
        E(r'(?<!TO )\d\d[A-Z]{3}\d\d *'),
    ...
    'location': (
        ...
        E('(?i)^(applegreen|boi|centra|circle k|dunnes|eurospar|gamestop|mcdonalds|michie sushi|pablo picante|park rite|penneys|pizza hut|polonez|spar|starbucks|supervalu|topaz|ubl|ulster bank|wh smith|zabka) +(.+)$',
